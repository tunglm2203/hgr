...
from baselines.her.rollout import RolloutWorkerRecord
import argparse
import gym
...
parser.add_argument('--video', type=str, default='FetchReach-v1')
...
def main(policy_file, seed, n_test_rollouts, render, video=None):
    ...
    evaluator = RolloutWorkerRecord(params['make_env'], policy, dims, logger, **eval_params,
                                    video_path=video)
    evaluator.seed(seed)
    ...
    evaluator.video_recorder.close()

    print('Success history: ', evaluator.success_history)
    # record logs
