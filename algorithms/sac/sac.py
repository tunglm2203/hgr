...
def sac(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
        ...
        max_ep_len=1000, logger_kwargs=dict(), save_freq=1, tensor_board=None):
    """
    ...
    for t in tqdm(range(total_steps)):
        ...
        if t > 0 and t % steps_per_epoch == 0:
            ...
            mean_loss_q1, _ = mpi_statistics_scalar(logger.epoch_dict['LossQ1'])
            mean_loss_q2, _ = mpi_statistics_scalar(logger.epoch_dict['LossQ2'])
            mean_loss_v, _ = mpi_statistics_scalar(logger.epoch_dict['LossV'])
            mean_q1_vals, _ = mpi_statistics_scalar(logger.epoch_dict['Q1Vals'])
            mean_q2_vals, _ = mpi_statistics_scalar(logger.epoch_dict['Q2Vals'])
            mean_vvals, _ = mpi_statistics_scalar(logger.epoch_dict['VVals'])
            mean_logpi, _ = mpi_statistics_scalar(logger.epoch_dict['LogPi'])
            ...
            logs = {'Return_mean': mean_ret, 'Return_std': std_ret, 'Return_max': max_ret, 'Return_min': min_ret,
                    ...
                    'Loss_pi': mean_loss_pi, 'Loss_Q1': mean_loss_q1, 'Loss_Q2': mean_loss_q2, 'Loss_V': mean_loss_v,
                    'Q1_values': mean_q1_vals, 'Q2_values': mean_q2_vals, 'V_values': mean_vvals,
                    'Log_pi': mean_logpi
                    }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', type=str, default='../logs/sac_test')
