...
def trpo(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
         ...
         save_freq=10, algo='trpo', tensor_board=None):
    """
    ...
    # obs_dim = env.observation_space.shape  # Modify for compatible with FetchEnv gym
    obs_dim = (env.observation_space.spaces['observation'].shape[0] +
               env.observation_space.spaces['achieved_goal'].shape[0], )
    obs_space_fake = Box(low=-1.0, high=1.0, shape=obs_dim)  # Fake env's configure for get dimension
    ...
    x_ph, a_ph = core.placeholders_from_spaces(obs_space_fake, env.action_space)
    ...
    for epoch in tqdm(range(epochs)):
        ...
        # TODO(tungluu) modify to view on TensorBoard
        mean_ret, std_ret, max_ret, min_ret = mpi_statistics_scalar(logger.epoch_dict['EpRet'], with_min_and_max=True)
        mean_len, _ = mpi_statistics_scalar(logger.epoch_dict['EpLen'], with_min_and_max=False)
        mean_val, std_val, max_val, min_val = mpi_statistics_scalar(logger.epoch_dict['VVals'], with_min_and_max=True)
        mean_loss_pi, _ = mpi_statistics_scalar(logger.epoch_dict['LossPi'], with_min_and_max=False)
        mean_loss_v, _ = mpi_statistics_scalar(logger.epoch_dict['LossV'], with_min_and_max=False)
        mean_delta_loss_pi, _ = mpi_statistics_scalar(logger.epoch_dict['DeltaLossPi'], with_min_and_max=False)
        mean_delta_loss_v, _ = mpi_statistics_scalar(logger.epoch_dict['DeltaLossV'], with_min_and_max=False)
        mean_kl, _ = mpi_statistics_scalar(logger.epoch_dict['KL'], with_min_and_max=False)

        logs = {'Return_mean': mean_ret, 'Return_std': std_ret, 'Return_max': max_ret, 'Return_min': min_ret,
                'Episode_len': mean_len, 'VValue_mean': mean_val, 'VValue_std': std_val, 'VValue_max': max_val,
                'VValue_min': min_val,
                'Loss_pi': mean_loss_pi, 'Loss_v': mean_loss_v,
                'Loss_pi_delta(new_old)': mean_delta_loss_pi, 'Loss_v_delta(new_old)': mean_delta_loss_v,
                'KL(old_new)': mean_kl
                }
...
if __name__ == '__main__':
    ...
    trpo(lambda: gym.make(args.env), actor_critic=core.mlp_actor_critic,
         ac_kwargs=dict(hidden_sizes=[args.hid] * args.l, activation=tf.nn.relu), gamma=args.gamma,
