    ...
    for env_id in sorted(data.keys()):
        ...
        for config in sorted(data[env_id].keys()):
            ...
            if args.plot_as_epoch:
                ...
                plt.plot(xs_data, np.nanmedian(ys, axis=0), label=config)
                plt.fill_between(xs_data, np.nanpercentile(ys, 25, axis=0), np.nanpercentile(ys, 75, axis=0), alpha=0.25)
        ...
        # plt.title(env_id + '(Varied size of buffer)', fontsize=10)
        # plt.title(env_id + '(Varied size of minibatch)', fontsize=10)
        plt.title(env_id, fontsize=10)
        plt.ylabel('Median Success Rate', fontsize=10)
        plt.ylim([0, 1.05])
